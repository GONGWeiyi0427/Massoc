...
`ifdef	FORMAL
	...
	// Q1: i_start_signal may be raised at any time
	always @(*)
		assert(counter < MAX_AMOUNT);

	// Q2: Once raised, assume i_start_signal will remain high until it is high and the counter is no longer busy.
	always @(*)
		if((i_start_signal) && (counter == 0))
			assume(i_start_signal);
			
	// Q3: o_busy will always be true while the counter is non-zero Make sure you check o_busy both when counter == 0 and counter != 0
	always @(*)
		if(!counter == 0)
			assert(o_busy);
		else
	...
	// Q4: If the counter is non-zero, it should always be counting down. Beware of the reset!
	always @(posedge i_clk)
		if(!$past(i_reset) && (counter != 0) && !$past(i_start_signal))
			assert($past(counter) == counter + 1'b1);
