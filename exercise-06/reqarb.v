...
`ifdef	FORMAL
// Q1: No data will be lost, no requests will be dropped. Assume all requests remain stable until accepted.
always @(posedge i_clk)
	if ((i_a_req) && (o_a_busy) && (!i_reset) && (!$past(i_reset)))
	begin
		assume($stable(i_a_req));
	end
always @(posedge i_clk)
	if ((i_b_req) && (o_b_busy) && (!i_reset) && (!$past(i_reset)))
	begin
		assume($stable(i_b_req));
	end

// Q2: Only one source ever gets access at a time. Assert one busy line is always high.
...
always @(*)
	if (!(i_busy))
		assert((!(o_a_busy) && o_b_busy) || (o_a_busy && !(o_b_busy)));

/*
// Q3: Therefore, all requests go through . . . eventually. This is a natural consequence of the above. Don’t worry about starvation here.
