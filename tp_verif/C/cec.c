   ...
   Only prints the clauses if print is nonzero. */
{
  ...
  for (i=0; i < c->wirecount; i++) {
    ...
    case OR:
      ...
      /* If all inputs are 0, the output must be 0. */
      for (j=0; j < c->wires[i]->fanincount; j++) {
		    print("%d ", c->wires[c->wires[i]->fanins[j]]->var_num);
      }
      print("-%d 0\n", c->wires[i]->var_num);
    ...
    case NOR:
      ...
      /* If all inputs are 0, the output must be 1. */
      for (j=0; j < c->wires[i]->fanincount; j++) {
		    print("%d ", c->wires[c->wires[i]->fanins[j]]->var_num);
      }
      print("%d 0\n", c->wires[i]->var_num);
...
int Test_1_circuit(char * circ)
{
  circuit circuit1;
  int i;
  int circuit1_clauses;
  /* Test pour 1 circuit */
  circuit1 = read_iscas85_file(circ);
  if (!circuit1) exit(1);

  //Give a DIMACS number to each wires
  for (i=0; i < circuit1->wirecount; i++)
    circuit1->wires[i]->var_num = i+1;
  // Nb de variables et de clauses
  printf("p cnf ");
  printf("%d ",circuit1->wirecount);
  circuit1_clauses = make_clauses(circuit1,no_printf);
  printf("%d\n",circuit1_clauses);
  // print the clauses
  circuit1_clauses = make_clauses(circuit1,printf);
  return 0;
}


int Test_equiv(char * circ1, char* circ2)
{
  circuit circuit1, circuit2;
  int i, j;
  int circuit1_clauses, circuit2_clauses;

  circuit1 = read_iscas85_file(circ1);
  if (!circuit1) exit(1);
  circuit2 = read_iscas85_file(circ2);
  if (!circuit2) exit(1);
  ...
  {
    printf("The two circuits dont have the same inputs\n");
    return 0;
  }else {
    for (i = 0 ; i < (circuit1 -> inputcount); i++)
    {
      int found = 0;
      for (j = 0 ; j < (circuit2 -> inputcount); j++)
      {
        if(!strcmp((circuit1 -> wires)[*(circuit1 -> inputs+i)] -> name, (circuit2 -> wires)[*(circuit2 -> inputs+j)] -> name)){
          found = 1; 
          break;
        }
      }
    ...
    printf("c The two circuits have the same inputs\n");
  }
  ...
  {
    printf("The two circuits dont have the same outputs\n");
    return 0;
  }else {
    for (i = 0 ; i < (circuit1 -> outputcount); i++)
    {
      int found = 0;
      for (j = 0 ; j < (circuit2 -> outputcount); j++)
      {
        if(!strcmp((circuit1 -> wires)[*(circuit1 -> outputs+i)] -> name, (circuit2 -> wires)[*(circuit2 -> outputs+j)] -> name)){
          found = 1; 
          break;
        }
      }
    ...
    printf("c The two circuits have the same outputs\n");
  }
  ...
  //TODO
  int clausecount_total = 0;
  clausecount_total += make_clauses(circuit1,printf);
  clausecount_total += make_clauses(circuit2,printf);
  ...
  for (i = 0 ; i < (circuit1 -> inputcount); i++)
  {
    ...
    {
      if(!strcmp((circuit1 -> wires)[*(circuit1 -> inputs+i)] -> name, (circuit2 -> wires)[*(circuit2 -> inputs+j)] -> name)){
        // i is the index of the wire of the circuit which is the same of the jth of the wire of the second circuit
        // need to create the two clause for each input
        int num_var1 = (circuit1 -> wires)[*(circuit1 -> inputs+i)] -> var_num;
        int num_var2 = (circuit1 -> wires)[*(circuit2 -> inputs+j)] -> var_num;
        // num_var1 & num_var2 are the variables for each wire in each circuit
        // The two clause are (a + !a')(!a + a')
        printf("%d -%d 0\n", num_var1, num_var2);
        printf("-%d %d 0\n", num_var1, num_var2);
        clausecount_total += 2;
        break;
  ...
  */
  //TODO
  int outputcmppointer = circuit1->wirecount + circuit2->wirecount + 1;
  for (i = 0 ; i < (circuit1 -> outputcount); i++)
  {
    for (j = 0 ; j < (circuit2 -> outputcount); j++)
    {
      if(!strcmp((circuit1 -> wires)[*(circuit1 -> outputs+i)] -> name, (circuit2 -> wires)[*(circuit2 -> outputs+j)] -> name)){
        // i is the index of the wire of the circuit which is the same of the jth of the wire of the second circuit
        // need to create the 4 clause for each input to do the function xor
        int num_var1 = (circuit1 -> wires)[*(circuit1 -> outputs+i)] -> var_num;
        int num_var2 = (circuit1 -> wires)[*(circuit2 -> outputs+j)] -> var_num;
        // num_var1 & num_var2 are the variables for each wire in each circuit
        // The two clause are (a + !a')(!a + a')
        printf("-%d -%d -%d 0\n", num_var1, num_var2, outputcmppointer+i);
        printf("%d %d -%d 0\n", num_var1, num_var2, outputcmppointer+i);
        printf("%d -%d %d 0\n", num_var1, num_var2, outputcmppointer+i);
        printf("-%d %d %d 0\n", num_var1, num_var2, outputcmppointer+i);
        clausecount_total += 4;
...
{
  //Test produce CNF
  // Test_1_circuit(argv[1]);
  //Test équivalence
  Test_equiv(argv[1],argv[2]);
